...
function longestSentence(text) {
  let wordsArrays = wordsArray(text);
  let sortedByLengths = sortedByLength(wordsArrays);
  return sortedByLengths[0].join(' ') + `\nThe longest sentence has ${sortedByLengths[0].length} words.`;
}

function wordsArray(text) {
  return text.split(/\w.*?[.!?]/g).map(sentence => {
    return sentence.split(' ');
  });
}

function sortedByLength(wordsArrays) {
  return wordsArrays.sort((a, b) => b.length - a.length)
}

// Lesson Solution (Haven't learned regular expressions yet):
function longestSentence(text) {
