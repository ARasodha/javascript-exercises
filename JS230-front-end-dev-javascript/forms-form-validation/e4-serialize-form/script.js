...
function getFormData() {
  let firstName = encodeURIComponent(document.querySelector("[name=first-name]").value);
  let lastName = encodeURIComponent(document.querySelector("[name=last-name]").value);
  let password = encodeURIComponent(document.querySelector("[name=password]").value);
  let phoneNumber = encodeURIComponent(document.querySelector("[name=phone-number]").value);
  let email = encodeURIComponent(document.querySelector("[name=email]").value);
  let ccInputs = Array.from(document.getElementsByClassName("ccn")).map(input => encodeURIComponent(input.value)).join('');
  console.log(ccInputs)
  return {first_name: firstName, last_name: lastName, password, phone_number: phoneNumber, email, cc: ccInputs};
}
...
function validateFormOnSubmit() {
  ...
  form.addEventListener('submit', event => {
    ...
    if (validations === true) {
      ...
      let submitInfoSection = document.getElementById("form-submit-info");
      let submitTemplate = Handlebars.compile(document.getElementById('form-submit-template').innerHTML);
      let data = getFormData();
      let urlFormatData = Object.entries(data).map(entry => entry.join('=')).join('&');
      submitInfoSection.innerHTML = submitTemplate({responseBody: urlFormatData});
